...
def predict_cases(model, list_of_lists, output_filenames, folds, save_npz, num_threads_preprocessing,
    ...
    list_for_preprocessing = []
    for i in range(len(list_of_lists)):
        list_for_preprocessing.append([list_of_lists[i][0]])
    preprocessing = preprocess_multithreaded(trainer, list_for_preprocessing, cleaned_output_files, num_threads_preprocessing,
    ...
    for preprocessed in preprocessing:
        ...
        '''load dist map'''
        for i in range(len(list_of_lists)):
            if list_of_lists[i][1][-20:-12] == output_filename[-15:-7]:
                dist_filename = list_of_lists[i][1]
        print('load dist map: ', dist_filename)
        dist = nib.load(dist_filename)
        dist = dist.get_fdata()
        dist = dist.transpose(2,0,1) # see if this one is correct!!
        # crop the dist with the same cropping as preprocessed images
        dist = dist[dct['crop_bbox'][0][0]:dct['crop_bbox'][0][1], dct['crop_bbox'][1][0]:dct['crop_bbox'][1][1], dct['crop_bbox'][2][0]:dct['crop_bbox'][2][1]]
        dist = dist[np.newaxis,:,:,:] # add one dimension at axis=0
        # resample the dist to the same size as the preprocessed images
        if not np.array_equal(np.array(dct['original_spacing']), np.array(dct['spacing_after_resampling'])):
            new_shape = np.round(((np.array(dct['original_spacing']) / np.array(dct['spacing_after_resampling'])).astype(float) * dct['size_after_cropping'])).astype(int)
            dist = resample_data_or_seg(dist, new_shape, False, axis=None, order=3, do_separate_z=False,
                                             order_z=0)
        # concatenate dist with image
        d = np.concatenate((d, dist), axis=0)
        ...
        '''for save prob map: uncomment the two lines below and comment results.append(..)'''
        # np.save(output_filename[:-7] + ".npy", softmax)
        # softmax = output_filename[:-7] + ".npy"
