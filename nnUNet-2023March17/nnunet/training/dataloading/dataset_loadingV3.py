...
class DataLoader3D(SlimDataLoaderBase):
    ...
    def generate_train_batch(self):
        ...
        for j, i in enumerate(selected_keys):
            ...
            data_ori = case_all_data[:-1].copy()
            seg_ori = case_all_data[-1:].copy()
            seg_ori_copy = seg_ori.copy()   
            dist = compute_edts_forPenalizedLoss(seg_ori_copy) 
            # dist = seg_ori.copy()
            dist_add1 = dist[np.newaxis,:,:,:] # add one dimension at axis=0
            data_dist = np.concatenate((data_ori, dist_add1), axis=0)
            # calculate new label - 1,2,3,4,5,6,7,8 - with dist threshold = 0.6
            label = seg_ori.copy()
            label = np.squeeze(label)
            labelpos = np.where(label==-1, 0, label)
            labelneg = np.where(label!=-1, 0, label)
            labelpos = np.where(labelpos==5, 0, labelpos)
            '''normalize the ratio based on each organ'''
            # new_dist_map = np.zeros(label.shape)
            for m in range(1,len(np.unique(labelpos))):
                labeltemp = np.copy(labelpos)
                ...
                # dist_sum = np.sum(new_dist_map)
                # dist_has_nan = np.isnan(dist_sum)
                # if dist_has_nan:
                #     print(i,'dist map has nan ', dist_has_nan)

            new_dist_map[new_dist_map<0.6] = 0 # set dist threshold = 0.6
            new_dist_map[new_dist_map>0] =1
            labelpos_dist = labelpos*new_dist_map # overlap dist map onto gt label
            labelpos_dist = np.where(labelpos_dist!=0, 4, labelpos_dist)
            labelpos_dist = labelpos + labelpos_dist # labeled near-target region as 5/6/7/8 for bladder, rectum, sigmoid, small bowel
            label_dist = labelpos_dist + labelneg # add back -1 label
            label_dist = label_dist[np.newaxis,:,:,:] # add one dimension at axis=0
            
            changed_all_data = np.zeros((case_all_data.shape[0]+1, case_all_data.shape[1], case_all_data.shape[2], case_all_data.shape[3]))
            changed_all_data[:-1] = data_dist
            changed_all_data[-1:] = label_dist
            ...
            for d in range(3):
                ...
                # always
                if need_to_pad[d] + changed_all_data.shape[d + 1] < self.patch_size[d]:
                    need_to_pad[d] = self.patch_size[d] - changed_all_data.shape[d + 1]

            # we can now choose the bbox from -need_to_pad // 2 to shape - patch_size + need_to_pad // 2. Here we
            # define what the upper and lower bound can be to then sample form them with np.random.randint
            shape = changed_all_data.shape[1:]
            ...
            changed_all_data = np.copy(changed_all_data[:, valid_bbox_x_lb:valid_bbox_x_ub,
            ...
            data[j] = np.pad(changed_all_data[:-1], ((0, 0),
                                                  (-min(0, bbox_x_lb), max(bbox_x_ub - shape[0], 0)),
                                                  (-min(0, bbox_y_lb), max(bbox_y_ub - shape[1], 0)),
                                                  (-min(0, bbox_z_lb), max(bbox_z_ub - shape[2], 0))),
                             self.pad_mode, **self.pad_kwargs_data)

            seg[j, 0] = np.pad(changed_all_data[-1:], ((0, 0),
                                                    (-min(0, bbox_x_lb), max(bbox_x_ub - shape[0], 0)),
                                                    (-min(0, bbox_y_lb), max(bbox_y_ub - shape[1], 0)),
                                                    (-min(0, bbox_z_lb), max(bbox_z_ub - shape[2], 0))),
